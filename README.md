...
* Время – $O(p^2)$ : <br>
  При вычислении $a_i$ для $i = p..1$ происходит суммирование $p - i + 1$ элементов – $O(1 + 2 + ... + p) = O(p^2)$. $a_{p+1}$ вычисляется за $O(1)$. Также предварительно нужно вычислить биномиальные коэффициенты за $O(p^2)$.
