...
* Время – $O(p^2)$ : <br> 
  При вычислении каждого $a_i$ ( $i \in [1..p]$ ) происходит суммирование $p - i + 1$ элементов – суммарно $O(1 + 2 + \ldots + p) = O(p^2)$, а $a_{p+1}$ вычисляется за $O(1)$. Предварительное вычисление биномиальных коэффициентов требует $O(p^2)$ времени.
